...
public int [] Maxparle= {-1,-1,-1,-1};
public int [] Active_x;
public int []Active_y;
public int [] send_Y=new int [4];
public int []checkpath= {-1,-1,-1,-1};
public int []validList= {-1,-1,-1,-1};
public int [] reset= {-1,-1,-1,-1};
public int parse=0;
public int [] sumkey=new int[4];
public int[]numberkey=new int [4];
...
private boolean setFlag1=false;
	...
	public void Parle_kid(int stop,int start) {
		...
		for(int i=start;i<stop;i++) {
			...
			if (Checkcontains(validList,Active_y[i])==false  &&  Checkcontains(checkpath,Active_x[i])==false) {
				...
				parse++;
				if (parse!=4) {Parle_kid(sumkey[parse],sumkey[parse-1]);}
			}
			if (parse==4 ) {
				parse=setindex(i);
				flagset=true;
	...
	public int softsearchNear(int [] checking) {
		...
		for (int i=0;i<checking.length;i++) {
			for (int j=0;j<stop;j++) {
				if (i==checking.length-1) {continue;}
				if (checking[i]==Too_Near[j][0] && checking[i+1]==Too_Near[j][1]) {result+=soft_Too_Near[j][2];}
			}
		}
		return result;
	}
